...
def main() -> None:
    ...
    ds_plugin = DeepSpeedPlugin(zero_stage=2,
                                gradient_accumulation_steps=pc.grad_accum # 这里已经指定了累积步数
                                )
    accelerator = Accelerator(
        log_with="tensorboard",
        project_dir=pc.log_dir,
        mixed_precision="fp16",
        gradient_accumulation_steps=pc.grad_accum,  # ← 新增
    ...
    try:
        ...
        for epoch in range(1, pc.epochs + 1):
            ...
            for batch in train_dl:
                ...
                loss = outputs.loss / pc.grad_accum # DeepSpeed 已做必要缩放
                accelerator.backward(loss)  # AMP + all-reduce
                if accelerator.sync_gradients:
                    # 在优化器 step 之前裁剪梯度，防止梯度过大
                    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0, norm_type=2.0)
                    optim.step()
                    sched.step()  # lr 调度在参数更新之后
                    optim.zero_grad()  # 累积完才清空
                    global_step += 1
                    ...
                    # 保存 + 评估
                    if accelerator.is_main_process and (global_step) % pc.save_freq == 0:
                        eval_loss = evaluate(model, dev_dl)
                        if eval_loss < best_loss:
                            best_loss = eval_loss
                            path = os.path.join(pc.save_dir, "best")
                            accelerator.wait_for_everyone()
                            # 当你用 DeepSpeedPlugin（或任何分布式包装器）包装模型后，model 实际上是一个 wrapper（封装对象），里面保存了原始模型和额外的分布式状态。
                            #
                            save_model(accelerator.unwrap_model(model), path)
                            tok.save_pretrained(path)
                            text1= f"✔ Saved best checkpoint to {path} (eval loss={eval_loss:.4f})"
                            # 文本
                            if writer is not None:
